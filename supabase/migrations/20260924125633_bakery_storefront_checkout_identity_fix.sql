...
begin
  ...
  insert into public.orders(tenant_id,status,fulfillment_type,currency,subtotal,discount_total,delivery_fee,total,customer_snapshot,fulfillment_snapshot,notes,source,idempotency_key,created_by)
  values(v_tenant_id,'pending',p_fulfillment_type,'BRL',v_subtotal,0,v_delivery_fee,v_total,coalesce(p_customer,'{}'::jsonb),coalesce(p_fulfillment,'{}'::jsonb),p_notes,'web',p_idempotency_key,auth.uid())
