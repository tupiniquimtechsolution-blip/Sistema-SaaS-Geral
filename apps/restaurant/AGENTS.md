...
- Limite mudanças ao escopo. Peça aprovação antes de exclusões, migrações irreversíveis, alteração de dados/schema reais, force-push, publicação externa ou dependência estrutural.
- Execute lint/typecheck/test/build e verificações de segurança quando disponíveis.

## Segurança
- Nunca exponha/versione secrets, tokens, cookies, senhas ou chaves privilegiadas.
- Auth/authz sensíveis no servidor; valide entradas; aplique rate limiting.
- Avalie XSS, CSRF, SQL/command injection, SSRF, path traversal e riscos específicos da stack.
...
## Gate web/pré-launch
Validar conteúdo real, mobile, acessibilidade, contatos clicáveis, overlays, peso de mídia, /admin não indexado e ausência de dados de desenvolvimento. Quando aplicável: sitemap/Search Console, Bing Webmaster, PageSpeed, títulos/metas únicos, links internos, analytics com consentimento e presença local.
