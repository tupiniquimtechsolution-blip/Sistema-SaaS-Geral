...
export function EntityArtCard({
  ...
  return (
    ...
    >
      {/* mobile — arte acima, texto abaixo */}
      ...
      {/* desktop — banner completo, texto na metade direita (área livre) */}
      <div className="relative hidden min-h-[360px] sm:block">
        ...
        />
        {/* véu apenas sobre o lado do texto */}
        <div
...
/* ---------------- Próximas atividades (Calendário) ---------------- */

/* ---------- mini-card ilustrado (3 grupos do calendário, §10) ---------- */
...
export function EntityArtMini({
  ...
  const inner = (
    <>
      {/* arte à esquerda — o texto nunca sobrepõe a figura (§9) */}
      {visual?.image ? (
        <img
          src={visual.image}
  ...
  return (
    <section aria-label="Próximas atividades" className="mt-12">
      <div className="grid gap-5 lg:grid-cols-3">
        {/* GIRAS — arte automática da entidade/linha cadastrada */}
        ...
        {/* DESENVOLVIMENTO — arte própria pendente; fallback institucional (§13) */}
        ...
        {/* CONSULTAS — arte da entidade associada (Maria Mulambo, §10) */}
...
/* ---------------- Linhas de trabalho (artes oficiais) ---------------- */
