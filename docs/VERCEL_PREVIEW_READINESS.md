...
| Storage security | PARTIAL | Buckets canônicos confirmados; harness cross-tenant de storage pendente |
...
1. **Storage cross-tenant = PARTIAL** — o harness de storage (upload assinado
   cross-tenant nas buckets `tenant-public`/`tenant-private`, path
   `<tenant-uuid>/<folder>/<file>`) deve ser implementado antes de qualquer
   preview com funcionalidade de upload.
